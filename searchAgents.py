...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.startingPosition, (0, 0, 0, 0)  # second item is used for visited corners checks
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hits_wall:
                new_visited_corners = tuple(
                    (next_x, next_y) == self.corners[i] or visited_corners[i] for i in range(0, 4))
                successors.append((((next_x, next_y), new_visited_corners), action, 1))
        self._expanded += 1  # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(0, 4):
        if not visited_corners[i]:
            # manhattan distance
            distance = abs(current_position[0] - corners[i][0]) + abs(current_position[1] - corners[i][1])
            furthest = max(distance, furthest)
    return furthest  # Default to trivial solution
